...
var lasersTwo = [];
var aboutSplashIMG, howSplashIMG, doneSplashIMG;
var done;
var winIMG
...
imperialTroopIMG = loadImage('tie phantom.png');
...
backgroundIMG = loadImage('bgFinal.gif');
backgroundIMG2 = loadImage('bgFinal2.gif');
...
aboutSplashIMG = loadImage('about copy.png');
howSplashIMG = loadImage('how copy.png')
winIMG = loadImage('BabyYodaCongratulations.gif');
}
...
howpop.addImage(howSplashIMG)
howpop.scale = 1.5;

aboutpop = createSprite(width/2,height/2)
aboutpop.addImage(aboutSplashIMG);
aboutpop.scale = 1.5;
...
if(back.mousePressed(()=>{
  ...
  for (var i = 0; i <lasersTwo.length; i++) {
    lasersTwo[i].destroy();
  }
...
if(gameState=="levelSwitchTo3") {
  ...
  setTimeout( function() { switchStateThree(); }, 4000);
}
...
if(gameState==="play"){
    ...
    image(scoreIMG, width-320, height-75, 240, 75);
    text("Lives: "+life, width-300, 50);
    text("Score: "+score, width-320, 793); 
  ...
  text("Lives: "+life, width-300, 50);
    ...
    collideFunctionTwo();
    gameOverAndLifeReductionLevelSevenToNine();
}
...
function createNewShip() {
  ...
  function shootTwo() {
    ...
    laserTwo.y = milleniumFalconTwo.y - 100;
  ...
  function collideFunctionTwo() {
    for(var i = 0; i<enemyShipNewGroup.length; i++) {
      for(var x = 0; x<lasersTwo.length; x++) {
    ...
    for(var i = 0; i<rocks.length; i++) {
      for(var x = 0; x<lasersTwo.length; x++) {
        if(rocks[i].isTouching(lasersTwo[x])) {
          score += 5;
          explosionSound.play();
          lasersTwo[x].destroy();
          rocks[i].addImage(explosionIMG);
          rocks[i].lifetime = 10;
          rocks[i].setCollider("circle", 2000000, 2000000, 2);
          rocks[i].scale = 0.7;
        }
      }
  }
